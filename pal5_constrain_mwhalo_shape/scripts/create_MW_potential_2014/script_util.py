...
# PROJECT : Pal 5 Constrain MW Halo Shape
#
# ----------------------------------------------------------------------------
...
def plot_mcmc_c(samples, fitvoro, cs, bf_params, add_families=False, savefig=None):
    ...
    bovy_plot.bovy_plot(
        [np.exp(bp[2]) * REFR0 for bp in bf_params], cs, "w-", overplot=True,
    )
    if add_families:
        bovy_plot.bovy_plot(
            np.exp(pot_params[2]) * REFR0, pot_params[7], overplot=True, **fm_kw,
        )
    nullfmt = NullFormatter()
    plt.gca().yaxis.set_major_formatter(nullfmt)
    axes_white()
    ...
    bovy_plot.bovy_plot(
        [np.exp(bp[3]) * REFR0 * 1000.0 for bp in bf_params], cs, "w-", overplot=True,
    )
    if add_families:
        bovy_plot.bovy_plot(
    ...
    bovy_plot.bovy_plot(
        [np.exp(bp[4]) * REFR0 for bp in bf_params], cs, "w-", overplot=True,
    )
    if add_families:
        bovy_plot.bovy_plot(
            np.exp(pot_params[4]) * REFR0, pot_params[7], overplot=True, **fm_kw,
        )
    nullfmt = NullFormatter()
    plt.gca().yaxis.set_major_formatter(nullfmt)
    axes_white()
...
def plotForceField(savefig: Optional[str] = None):
    ...
    ro, vo = REFR0, REFV0
    pot = setup_potential(p_b15, p_b15_pal5gd1_voro[0][-1], False, False, ro, vo)
    # Compute force field
    ...
    for angle in tqdm(np.linspace(0.0, np.pi / 2.0, 8)):
        plt.plot((0.0, 100.0 * np.cos(angle)), (0.0, 100.0 * np.sin(angle)), "k:")
        plt.plot((0.0, 100.0 * np.cos(angle)), (0.0, -100.0 * np.sin(angle)), "k:")
    # Add measurements
    # Pal 5
    plt.quiver((8.0,), (16.0,), (-0.8,), (-1.82,), color="w", zorder=10, scale=scale)
    # GD-1
    plt.quiver(
        (12.5,), (6.675,), (-2.51,), (-1.47,), color="w", zorder=10, scale=scale,
    )
    ...
    bovy_plot.bovy_text(8.65, 0.5, r"$\mathbf{disk\ stars}$", color="w", size=17.0)
