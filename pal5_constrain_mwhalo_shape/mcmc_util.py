...
def read_mcmc_chain(chain, drct="pal5_mcmc", return_potparams=True, permute=False):
...
def pm_pal5(chain_params):
    ...
    """
    pmra = -2.296 + chain_params[3] + chain_params[4]
    pmdec = -2.257 + (_PMDECPAR * chain_params[3]) + (_PMDECPERP * chain_params[4])
    return (pmra, pmdec)
...
def _setup_model_tracks(
    ...
    pot, sdf_t, sdf_l = setup_model(
        potparams, chain_params[chain_index], *setup_args, num_t=2000, **setup_kw,
...
def setup_models_in_chain(
    ...
    # read_mcmc_chain
    potparams, chain_params = read_mcmc_chain(index, *mcmc_args, direc=direc, **mcmc_kw)
    # logger.verbort(
...
def plot_chains(drct, nwalkers=12, ffmt=".dat"):
    ...
    for en, (ii, fn) in enumerate(zip(range(len(chains)), chains)):
        ...
        sdata = np.reshape(data[:, -1], (len(data[:, 5]) // nwalkers, nwalkers))
